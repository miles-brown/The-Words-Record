/**
 * Migration script to convert existing markdown content to database records
 * Run with: npx ts-node scripts/migrate-markdown.ts
 */
...
async function createPersonIfNotExists(name: string): Promise<string> {
...
async function createTagIfNotExists(tagName: string): Promise<string> {
...
async function migrateMarkdownCase(caseData: MarkdownCase): Promise<void> {
  try {
    ...
    if (caseData.sources && caseData.sources.length > 0) {
      await Promise.all(
        caseData.sources.map(async (sourceTitle, index) => {
...
async function loadMarkdownCases(): Promise<MarkdownCase[]> {
...
async function createSampleData(): Promise<MarkdownCase[]> {
  return [
    {
...
export { main as migrateMarkdown }