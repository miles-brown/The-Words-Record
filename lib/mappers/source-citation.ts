...
import type { Source, MediaOutlet, Journalist } from '@prisma/client'
...
export function sourceToHarvardCitation(source: SourceWithRelations): string {
  ...
  } else if (source.sourceType === 'SOCIAL_MEDIA') {
...
export function sourceToInTextCitation(source: SourceWithRelations): string {
  ...
  if (source.author) {
    ...
    const authorParts = source.journalist.name.split(' ')
    author = authorParts[authorParts.length - 1]
  } else if (source.mediaOutlet) {
    author = source.mediaOutlet.name
  }
...
export function sourceToShortCitation(source: SourceWithRelations): string {
  const author = source.author || source.journalist?.name || source.mediaOutlet?.name || 'Anonymous'
  const publication = source.publication || source.mediaOutlet?.name
  const year = source.publishDate
    ? new Date(source.publishDate).getFullYear()
    : 'n.d.'
